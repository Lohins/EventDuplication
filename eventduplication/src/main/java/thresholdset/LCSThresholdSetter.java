...
import simAlgorithm.CosineSimilarity;
import simAlgorithm.LCSSimilarity;
import text.eventduplication.xmlparse.EventXmlParser;

public class LCSThresholdSetter {

	public LCSThresholdSetter() {
	...
	public void generateThreshold( ArrayList<String[]> events, int index, String tag, double start_threshold, double step ) {
		...
		String excelName = saveFolder +  "\\LCS-" + tag + ".xlsx"; 
		...
		for (int i = 0; i < events.size(); i++){
			for (int k = 1; k < events.size(); k++){
				...
				double similarity = lcsSimilarity.similarity(text1, text2);
	...
	public static void main(String[] args) {
		...
		LCSThresholdSetter setter = new LCSThresholdSetter();
